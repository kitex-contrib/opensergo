...
func (o *OpenSergoMetaReporter) ReportMetaInfo(svcs map[string]*serviceinfo.ServiceInfo) error {
	if !o.enable {
		klog.Warn(util.ErrConfigNotFound.Error())
		return util.ErrConfigNotFound
	}
	metaReqs, err := o.openSergoMetaReq(svcs)
	if err != nil {
		klog.Errorf("err:%+v", err)
		return err
	}
	for _, metaReq := range metaReqs {
...
func (o *OpenSergoMetaReporter) openSergoMetaReq(svcs map[string]*serviceinfo.ServiceInfo) ([]*v1.ReportMetadataRequest, error) {
	var metaReqs []*v1.ReportMetadataRequest
	for _, srvInfo := range svcs {
